...
```
 ...
 ```
- [x]  
对中断描述符表（IDT）进行初始化，其中包括了针对时钟中断的中断描述符的设置，即为其设置中断号。
为8259中断控制器进行初始化，利用lidt指令，告知cpu完成中断描述符的加载。 
为8253时钟外设的初始化，使之每100个tick产生一个中断。
>  
...
lab1中完成了对哪些外设的访问？ (w2l2)
 ...
 ```
- [x]  时钟，串口，并口，键盘，CGA等
>>>>>>> 6d1bf219ea186cf4a83951a69152fd8ae57bc1e1
...
- [x]  
第一步：实现一个能够显示字符串的bootloader，关键步骤是先将CPU从实模式切换到保护模式， 即通过修改A20地址线完成从实模式到保护模式的转换。然后要启动分段存储管理机制，只有在保护模式下才能使用分段存储管理机制。 其中需要注意4个关键内容：逻辑地址、段描述符（描述段的属性）、段描述符表（包含多个段描述符的“数组”）、 段选择子（段寄存器，用于定位段描述符表中表项的索引）。完成这些内容的建立和初始化，是bootloader的关键。
第二步：实现一个可加载ELF格式文件的bootloader，其需要完成的工作是从硬盘上加载并运行OS。LBA模式的PIO（Program IO）方式， 即所有的IO操作是通过CPU访问硬盘的IO地址寄存器完成，来访问硬盘。这一步实现需要注意理解ELF文件格式。 ELF格式是Linux系统下的一种常用目标文件(object file)格式，能够用于执行的可执行文件(executable file)，用于提供程序的进程映像， 加载的内存执行。
...
- [x]  使用strace函数，参数是-c是，可以统计每次系统调用都有一次记录，统计每种系统调用的次数即可。
